...
public class PostDaoImpl implements PostDao, PostCommentDao {
	...
	public void modifyTitle(int pNo, String title) {
		...
		jdbcTemplate.update("update post set title = ? where pno = ?", new Object[] { title, pNo });
	...
	public Post getPostInfo(int no) {
		...
		return jdbcTemplate.query(
				"select pt.*, m.nickname from (select row_number() over (order by wdate desc) no, p.* from post p where p.state = 1) pt, member m where no = ? and m.id = pt.id",
